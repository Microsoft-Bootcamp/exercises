# 2.3 Setting up Code Scanning and Secret Scanning for your repository
...
CodeQL requires a build of compiled languages, and an analysis job can fail if our autobuilder is unable to build a program to extract an analysis database. Head over to the C# job and determine if there's a build failure. Comment out the autobuild step and add the following 
```yaml
    - name: Setup dotnet
